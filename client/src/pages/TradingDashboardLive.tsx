...
export default function TradingDashboardLive() {
  const [selectedSymbol, setSelectedSymbol] = useState('EUR/USD');
  const [isTraining, setIsTraining] = useState(false);
  const [isBacktesting, setIsBacktesting] = useState(false);
  const [priceHistory, setPriceHistory] = useState<
    Array<{ time: number; price: number; bid: number; ask: number }>
  >([]);

  // WebSocket price stream
  const { prices, isConnected, error: wsError, getPrice } = usePriceStream([
    'EUR/USD',
    'GBP/USD',
    'USD/JPY',
    'USD/CHF',
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto p-4 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                LSTM-based forex trading with real-time WebSocket price feeds
              </p>
            </div>
        ...
        <Card className="border-accent">
          <CardHeader>
            <CardTitle className="text-accent">Live Price - {selectedSymbol}</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            {priceHistory.length > 1 && (
              <div className="mt-6 bg-card border border-border rounded-lg p-4">
                <h3 className="text-lg font-semibold text-accent mb-4">Price Movement</h3>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={priceHistory}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#333" />
                    <XAxis dataKey="time" stroke="#666" />
                    <YAxis stroke="#666" domain={['dataMin - 0.0001', 'dataMax']} />
                    <Tooltip
